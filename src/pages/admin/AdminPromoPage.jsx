...
export default function AdminPromoPage() {
  ...
  const submit = async (event) => { event.preventDefault(); if (!clean(form.title_id)) { setError("Judul promosi Indonesia wajib diisi."); return } if (form.start_date && form.end_date && form.end_date < form.start_date) { setError("Tanggal akhir tidak boleh sebelum tanggal mulai."); return } setSaving(true); setError(""); try { const payload = { ...form, title_id: clean(form.title_id), title_en: clean(form.title_en) || null, title_zh: clean(form.title_zh) || null, description_id: clean(form.description_id) || null, description_en: clean(form.description_en) || null, description_zh: clean(form.description_zh) || null, discount_text_id: clean(form.discount_text_id) || null, discount_text_en: clean(form.discount_text_en) || null, discount_text_zh: clean(form.discount_text_zh) || null, image_url: clean(form.image_url) || null, discount_text: clean(form.discount_text) || null, start_date: form.start_date || null, end_date: form.end_date || null, sort_order: Number(form.sort_order) || 0 }; if (editingId) await updatePromotion(editingId, payload); else await createPromotion(payload); setSuccess(editingId ? "Promosi diperbarui." : "Promosi dibuat."); setEditingId(""); setForm(EMPTY); await load() } catch (err) { setError(err.message || "Gagal menyimpan promosi.") } finally { setSaving(false) } }
  ...
  const toggleActive = async (item) => {
    ...
    try {
      await updatePromotion(item.id, { is_active: nextStatus })
      setSuccess(nextStatus ? "Promosi ditampilkan." : "Promosi disembunyikan.")
      await load()
    } catch (err) {
      setError(err.message || "Gagal mengubah status promosi.")
    }
  }
